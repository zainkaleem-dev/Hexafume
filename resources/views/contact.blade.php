    ...
    <div class="socials-row">
      @if(isset($socials['items']))
        @foreach($socials['items'] as $s)
          @php($platform = strtolower($s['platform'] ?? ''))
          @if(in_array($platform, ['instagram', 'linkedin'], true))
            <a href="{{ $s['link'] }}" class="social-btn" aria-label="{{ $s['platform'] }}" target="_blank" rel="noopener noreferrer">
              @if($platform === 'linkedin') <svg viewBox="0 0 24 24"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"/><circle cx="4" cy="4" r="2"/></svg>
              @else <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg> @endif
            </a>
          @endif
        @endforeach
      @endif
